...
@Component({
  ...
  imports: [NavbarComponent, MetricsComponent]
})
  ...
  drawTrees() {
    ...
    Promise.all([
      ...
      this.treesData.forEach((tree) => {
        ...
        ctx.drawImage(treeToRender, 0, 0, 170, 142, tree.posX, tree.posY + 150, 130, 122);
      });
      if (this.first < 3) {
        this.first += 1;
  ...
  test($event: any) {
    ...
    this.dialog
      ...
      .subscribe((result: any) => {
        ...
        this.environmental = (
          this.loadedCompanies.map((data) => data.environmental).reduce((a, b) => a + b, 0) /
          this.loadedCompanies.length
        ).toFixed(2);
        this.social = (
          this.loadedCompanies.map((data) => data.social).reduce((a, b) => a + b, 0) /
          this.loadedCompanies.length
        ).toFixed(2);
        this.governance = (
          this.loadedCompanies.map((data) => data.governance).reduce((a, b) => a + b, 0) /
          this.loadedCompanies.length
        ).toFixed(2);
        this.revenue = (
          this.loadedCompanies.map((data) => data.revenue).reduce((a, b) => a + b, 0) /
          this.loadedCompanies.length
        ).toFixed(2);

        this.cloudsData = [
          ...this.cloudsData,
          ...this.cloudDataBase.slice(this.cloudsData.length, this.loadedCompanies.length * 3)
        ];

        this.treesData = [
          ...this.treesData,
          ...this.treesDatabase.slice(this.treesData.length, this.loadedCompanies.length * 3)
        ];

        this.animalsData = [
          ...this.animalsData,
          ...this.animalsDatabase.slice(this.animalsData.length, this.loadedCompanies.length * 3)
        ];
